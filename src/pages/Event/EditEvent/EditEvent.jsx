...
function EditEvent() {
  ...
  async function fetchEvent() {
    ...
    toggleForm.setValues({
      ...
      guest: data.event.meta_data && data.event.meta_data.guest,
      winner: data.event.meta_data && data.event.meta_data.winner,
      outcome: data.event.meta_data && data.event.meta_data.outcome,
    });

    setGallery(data.event.gallery);
    setGuests(data.event.sections.guests);
  }
  ...
  const deleteGuest = async (index)=>{
    console.log(index)
    if(window.confirm("Are you sure you want to remove this guest")==true){
      let newguests = [];
  ...
  return (
    <Box elevation={2} sx={{ p: "2rem 3rem", mt: "2rem" }}>
      <Header
        title={"Edit Event"}
        subtitle={"Edit you event and view data about event"}
      />
      <Stack>
        {/* Basic Setting */}
        <Divider sx={{ mb: "2rem" }} />
            ...
            <Grid container spacing={2}>
              <Grid item xs={5}>
                <TextField
                  ...
                  error={
                    basicForm.touched.name && Boolean(basicForm.errors.name)
                  }
              ...
              <Grid item xs={3}>
                <MobileDateTimePicker
                  ...
                  error={
                    basicForm.touched.event_start &&
                    Boolean(basicForm.errors.event_start)
                  }
                  helperText={
                    basicForm.touched.event_start &&
                    basicForm.errors.event_start
                  }
                  slotProps={{
                    textField: {
                      fullWidth: true,
                      color: "secondary",
                      value: moment(basicForm.values.event_start),
                    },
                  }}
                />
              </Grid>
              <Grid item xs={3}>
                <MobileDateTimePicker
                  ...
                  error={
                    basicForm.touched.event_end &&
                    Boolean(basicForm.errors.event_end)
                  }
                  helperText={
                    basicForm.touched.event_end && basicForm.errors.event_end
                  }
                  slotProps={{
                    textField: {
                      fullWidth: true,
                      color: "secondary",
                      value: moment(basicForm.values.event_end),
                    },
                  }}
                />
              </Grid>
              <Grid item xs={12}>
                <TextField
                  ...
                  onChange={basicForm.handleChange}
                  error={
                    basicForm.touched.description &&
                    Boolean(basicForm.errors.description)
                  }
                  helperText={
                    basicForm.touched.description &&
        ...
        <form onSubmit={toggleForm.handleSubmit} style={{ marginTop: "1rem" }}>
          <Typography component={"label"} variant="body2" htmlFor="draft">
            Published
          </Typography>
          <Switch
            checked={!toggleForm.values.draft}
            color="secondary"
            value={!toggleForm.values.draft}
            onChange={(e) => {
              e.target.checked = !e.target.checked;
              e.target.value = !e.target.checked;
              toggleForm.handleChange(e);
            }}
            id="draft"
            name="draft"
          />
          <br />
          <Typography component={"label"} variant="body2" htmlFor="carousel">
            In Carousel
          </Typography>
          <Switch
            checked={toggleForm.values.carousel}
            color="secondary"
            value={toggleForm.values.carousel}
            onChange={toggleForm.handleChange}
            id="carousel"
            name="carousel"
          />
          <br />
          <Typography component={"label"} variant="body2" htmlFor="guest">
            Guest Section
          </Typography>
          <Switch
            checked={toggleForm.values.guest}
            color="secondary"
            value={toggleForm.values.guest}
            onChange={toggleForm.handleChange}
            id="guest"
            name="guest"
          />
          <br />
          <Typography component={"label"} variant="body2" htmlFor="winner">
            Winner Section
          </Typography>
          <Switch
            checked={toggleForm.values.winner}
            color="secondary"
            value={toggleForm.values.winner}
            onChange={toggleForm.handleChange}
            id="winner"
            name="winner"
          />
          <br />
          <Typography component={"label"} variant="body2" htmlFor="outcome">
            Outcome Section
          </Typography>
          <Switch
            checked={toggleForm.values.outcome}
            color="secondary"
            value={toggleForm.values.outcome}
            onChange={toggleForm.handleChange}
            id="outcome"
            name="outcome"
          />
        ...
        <input
          ...
          onChange={(e) => {
            addImage(document.getElementById("image-file").files[0]).then(
              (res) => {
        ...
        {gallery.length > 0 ? (
          <ImageList rowHeight={3} sx={{ width: "100%", height: "80vh" }}>
            {gallery.map((url) => {
              return (
          ...
          <Typography variant="body1"> No image added upload some</Typography>
        )}

        {/** Guest **/}
        {toggleForm.values.guest && (
          <>
            <Divider sx={{ mt: "1rem", mb: "1rem" }} />
            ...
            <Stack direction={"row"} gap="1rem">
              {guests.length == 0 && (
                <Typography>No Guest is added yet!</Typography>
              )}
                ...
                return (
                  <Box>
                    <ProfileCard
                      ...
                      name={guest.name}
                      profile={
                        process.env.REACT_APP_API_URL + "/" + guest.img_url
                      }
                      subtitle={guest.position}
                    />
                    <IconButton sx={{width:"100%", display:'flex', justifyContent:'center', borderRadius:'0%', bgcolor:'rgb(30,0,0)'}} color='error'
